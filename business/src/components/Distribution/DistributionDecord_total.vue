<template>
    <div class="tab-content">
        <ul class="block">
            <li>
                <span class="demonstration">商户订单</span>
                <el-input
                        placeholder="请输入订单号"
        ...
        <div class="box_table">
            <el-table v-loading="loadListing" :data="DistributionRecordS" border style="font-size:0.8em;">
                ...
                <el-table-column prop="colour_sn" label="彩之云订单号码" width="200">
                </el-table-column>
                <el-table-column prop="out_trade_no" label="商户订单号码" width="120">
                </el-table-column>
                <el-table-column prop="total_amount" label="订单金额" width="100">
                </el-table-column>
                <el-table-column prop="real_amount" label="订单实付金额" width="100">
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <el-popover trigger="hover" placement="top">
                      ...
                      <div slot="reference" class="name-wrapper">
                        ...
                        <el-tag color="#EDE879" size="small" v-if="scope.row.state==3">已处理</el-tag>
                        <el-tag color="#20A0FF" size="small" v-if="scope.row.state==4">回滚未处理</el-tag>
                        <el-tag color="#93031C" size="small" v-if="scope.row.state==5">回滚已处理</el-tag>
                      </div>
                    </el-popover>
                  </template>
                </el-table-column>
                <el-table-column prop="tag_name" label="分账科目"  width="80">
                </el-table-column>
                <el-table-column label="操作" width="200">
                    <template slot-scope="scope">
                        <el-button @click="details_Click(scope.$index, scope.row)" type="text" size="small">订单列表详情</el-button>
                        <!--<el-button @click="rule_details(scope.$index, scope.row)" type="text" size="small">对应规则</el-button>-->
                    </template>
                </el-table-column>
            </el-table>
            <!--分账详情-->
...
<script>
    import {TagOrderList,SplitLogList,TagList,backendOrderList} from './../../api/api';
    export default {
        props: {
        },
        data() {
...
//            this.tagList();
        ...
        methods: {
            ...
            rule_details(index,row){
//                console.log(row);
              this.$router.push({path: '/details', query:{map_id:row.map_id,business_uuid:row.business_uuid}});
            },
//            tagList(){//科目列表
...
//                        tag_uuid:this.SubjectsID,
                  ...
                  backendOrderList(SplitLogParams).then(data=>{
                        if(data.code==0){
            ...
            details_Click(index,row){
              let _this = this;
              console.log(row);
              _this.$router.push({ path: '/next_list',query:{business_uuid:row.business_uuid,order_source_id:row.id} });
