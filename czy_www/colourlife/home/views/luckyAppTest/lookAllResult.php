...
<title>查看战绩</title>
<link href="<?php echo F::getStaticsUrl('/common/css/lucky/june/lottery.css?time=123456'); ?>" rel="stylesheet">
   ...
   <div class="look_all_result">
     ...
     <div class="some_a_result">
       ...
       <div class="some_a_result_content">
         ...
         <p class="greenfont">根据您的竞猜战绩，距离"<?php echo $redPacket; ?>元"红包还差猜中<?php echo $lack; ?>场比赛，加油哦！猜对越多奖励越高！</p>
         <?php if($all){ ?>
         <div class="caipiao_box">
           <a href="/luckyApp/lookAllResult?all=all" class="rulebtn">查看详情</a>
         </div>
         <?php } ?>
         <table class="game_info" id="guessOutcome">
           <thead>
             <tr>
           ...
           <tbody>
             ...
             <tr>
               <td>
                <?php if($record->encounter->game_number<=48){ ?>
                 小组赛
                 <?php }else if($record->encounter->game_number>48 && $record->encounter->game_number<=56){ ?>
                 1/8决赛
                 <?php }else if($record->encounter->game_number>56 && $record->encounter->game_number<=60){ ?>
                 1/4决赛
                 <?php }else if($record->encounter->game_number>60 && $record->encounter->game_number<=62){ ?>
                 半决赛
                 <?php }else if($record->encounter->game_number==63){ ?>
                 三四名决赛
                 <?php }else if($record->encounter->game_number==64){ ?>
                 决赛
                 <?php } ?>
               </td>
               <td>
               ...
               <td>
                   ...
                   <?php }else if($record->encounter->outcome == 3){ ?>
                        <?php echo $record->encounter->HomeTeam; ?><span class="orange">平</span><?php echo $record->encounter->GuestTeam; ?>
                   <?php }else{ ?>
                        等待比赛结果
                   <?php } ?>     
               </td>
     ...
     <a href="/luckyApp/luckyApp" class="rulebtn">返回</a>      
