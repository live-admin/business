...
<html>
	...
	<body style="background: #f7f7f7;">
		...
		<div class="ranking_bar">
			...
			<ul class="drink"><div class="clear"></div></ul>
...
//		var _time =1486528800;
		...
		var goodlist=[$(".rich"),$(".drink"),$(".food"),$(".fruit")];
		for(var k in goods){
				addGood(goods[k],goodlist[k-1]);
			}
		
//		addGood(goods["1"],$(".rich"));
//		addGood(goods["2"],$(".drink"));
//		addGood(goods["3"],$(".food"));
//		addGood(goods["4"],$(".fruit"));
		...
		function addGood(obj,ele){
			...
			ele.append(
								...
								'<div class="rnk_right">'+
									...
									'<div class="num_box">'+
										'<div class="site_bot_box1a">'+
											'<span class="span_left">'+'已抢'+'<span>'+obj[i].number+'</span>'+'件'+'</span><span><span >'+(Math.round(obj[i].number/obj[i].amount*10000)/100)+'</span>'+'%'+'</span>'+
											'<div class="clear"></div>'+
											'<p><span class="plan" style="width:'+(Math.round(obj[i].number/obj[i].amount*10000)/100)+'%'+'"></span></p>'+
										'</div>'+
				...
				if (parseInt(obj[i].amount) <= parseInt(obj[i].number)) {
					var index = ele.index();
		...
		function showTime(){
			...
			if (h<10) {
				...
				$(".food,.fruit").delegate("a","click",function(){
				...
				$(".fruit").delegate("a","click",function(){
					$(".Pop_qiang_txt>p:eq(0)").text("亲，本场秒杀时间");
					$(".Pop_qiang_txt>p:eq(1)").text("还未到哦，请稍后再来！");
					$(".Pop_qiang").removeClass("hide");
					$(".mask").removeClass("hide");
				...
				$(".rich,.drink").delegate("a","click",function(){
					$(".Pop_qiang_txt>p:eq(0)").text("亲，本场秒杀已结束");
					$(".Pop_qiang_txt>p:eq(1)").text("下场秒杀请提早参加哦！");
					$(".Pop_qiang").removeClass("hide");
					$(".mask").removeClass("hide");
				...
				$(".food,.drink,.rich").delegate("a","click",function(){
					$(".Pop_qiang_txt>p:eq(0)").text("亲，本场秒杀已结束");
					$(".Pop_qiang_txt>p:eq(1)").text("下场秒杀请提早参加哦！");
					$(".Pop_qiang").removeClass("hide");
					$(".mask").removeClass("hide");
				...
				$(".top_nav>ul>li:eq(0)>p:eq(1)").text("已开抢");
				$(".top_nav>ul>li:eq(1)>p:eq(1)").text("已开抢");
				$(".top_nav>ul>li:eq(2)>p:eq(1)").text("已开抢");
				$(".top_nav>ul>li:eq(3)>p:eq(1)").text("抢购进行中");
				$(".drink").addClass("hide");
				$(".fruit").removeClass("hide");
				$(".top_nav ul li:eq(3)").addClass("choose");
				$(".top_nav ul li:eq(3)").css("color","#fff");
			}
