...
<div class="lottery_topic">
   <div class="lottery" id="lottery">
     ...
     <p class="warmTip" style="color:#0E4815; width:90%; margin:20px auto;">
         截至5月28日，全民抢粽子活动已经结束了，恭喜所有抢到粽子的业主们！没有抢到粽子的业主们，也请不要灰心，百万红包抽奖仍在进行，业主们可继续关注参与，马上抽奖，马上有钱！
     </p>
     <?php } ?>
     ...
     <div class="lottery_box zongzi_box">
      ...
      <div class="during">         
        <p class="during_next">
          本场剩余：<span class="nexttime" id="remaining"><?php if($able == 1){ echo $remaining;}else{ echo "0"; } ?></span>份五芳斋粽子
          <br />
   ...
   <div class="opacity" style="display:none;">
     ...
     <div class="grab_over" id="grab_over2" style="display:none;">
       <div class="alertcontairn_content">
         ...
         <div class="pop_btn">
           <a href="/luckyApp/myDumplings"><span>我抢到的粽子</span></a>  
...
function rob(){
    ...
    $.post(
        ...
        function (data){
            if(data == 1){
                $('.opacity').show();
                $('#grab_over4').show();   //抢到粽子
            }else if(data == 2){
                $('.opacity').show();
                $("#grab_over2").show();    //今天已抢到粽子，不能再抢
                $('#rob_grab').attr('class','grab no_grab');
                //$('#rob_grab').attr('href',"javascript:void()");
            }else{
                $('.opacity').show();
                $('#grab_over3').show();    //粽子抢光了
                $('#rob_grab').attr('class','grab no_grab');
                //$('#rob_grab').attr('href',"javascript:void()");
            }
        } 
...
$('.addresslink').click(
    function (){
        $.post(
            '/robRiceDumplings/getCustomerInfo',
            function (data){
...
$('#sure_address').click(
   function (){
       ...
       var tel = $('#tel').val();
       $.post(
           '/robRiceDumplings/fillReceiving',
