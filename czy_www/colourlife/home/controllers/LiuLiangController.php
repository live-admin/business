 ...
 * 微信分享送流量
 */
class LiuLiangController extends ActivityController{
    public $beginTime='2016-08-11';//活动开始时间
    public $endTime='2016-11-30 23:59:59';//活动结束时间      
...
//    }
    ...
    public function actionIndex(){
		...
		$url=F::getHomeUrl('/LiuLiang/ShareWeb').'?ts='.$time.'&sign='.$sign;
//        dump($url);
    ...
    public function actionShareWeb(){
        ...
        if(!empty(Yii::app()->session['wx_user']['openid'])){
           $openId= Yii::app()->session['wx_user']['openid'];
            ...
            $time=time();
            $sign=md5('share=ShareWeb&ts='.$time);
            $urlOther=F::getHomeUrl('/LiuLiang/ShareWeb').'?ts='.$time.'&sign='.$sign;
            $reUrl=base64_encode($urlOther);
            $url=F::getHomeUrl().'LiuLiang/Share?reUrl='.$reUrl.'&isWx=1';
//            dump($url);
            $this->redirect($url);
        }
        //测试使用
