<?php
class LuckyAppWebController extends CController {
 ...
 //        // if(!$this->isstart()){
 //        if(date('Y-m-d H:i:s')<='2015-01-31 23:59:59'){
 //            $layoutsList=array();            
 ...
 //                $this->renderPartial( "christmas_web",
 ...
 //    }
    ...
    public function actionBackIndex() {
        ...
        $allJoin=LuckyCustomerOut::model()->count();
        //判断活动是否结束
        if(date('Y-m-d H:i:s')>='2015-02-06 23:59:59'){
            $this->renderPartial( "happiness_web",
                array ( "luckyCustCan" => $this->_luckyCustCan,
                    "luckyTodayCan" => $this->_luckyTodayCan,
                    "allJoin"=>$allJoin+123,
                    "layoutList"=>$layoutsList,
    ...
    public function actionIndex() {
        $this->checkLogin();
        $this->renderPartial( "indexCarWeb" );
    }
    ...
    public function actionHappinessResult() {
        ...
        $this->renderPartial("happiness_resultWeb", array ("listResutlist"=>$listResutlist,"list"=>$list));
    }
    ...
    public function actionHappinessRule() {
        $this->checkLogin();
        $this->renderPartial("happiness_ruleWeb");
    }

    public function actionHappinessSnh() {
        $this->checkLogin();
        $this->renderPartial("happiness_snhWeb");
    }
...
// 		Yii::log("======".json_encode($b),CLogger::LEVEL_ERROR);
    ...
    public function actionMyoutcome(){
        ...
        $encounter_game=intval($encounter_game);
        //查看活动是否存在
	...
	public function actionView(){
		...
		$template = array('test', 'rules', 'shanjiaozhou', 'shuoming_5000', 'shuoming_fengchidao', 'shuoming_haoya', 'shuoming_huameida', 'shuoming_lizhi', 'shuoming_luofushang', 'shuoming_qingfeng', 'shuoming_quyuan', 'shuoming_taihu', 'shuoming_wine', 'shuoming_xunliaowan', 'shuoming_yijing', 'tai_hu', 'yi_jing', 'fengchidao', 'hao_ya', 'huameida', 'lizhi', 'luofushang', 'qing_feng', 'quyuan');
		if (!array_search($tpl, $template)) throw new CHttpException(404, '抱歉，找不到你请求的页面！');
		$array = array();
		$this->renderPartial('christmas/' . $tpl, $array);
	...
	public function actionChristmasResultWeb(){
        ...
        $listResutl = $this->getListData();    
        $list=LuckyCustResult::model()->findAll("cust_id=".$this->_userId." and lucky_act_id in (".Item::LUCKY_ACT_ID."-4,".Item::LUCKY_ACT_ID."-3,".Item::LUCKY_ACT_ID."-2,".Item::LUCKY_ACT_ID."-1,".Item::LUCKY_ACT_ID.") and prize_id!=".Item::LUCKY_THANKS_ID." order by id desc");
        $this->renderPartial("christmas/christmas_result", array ("listResutl"=>$listResutl,"list"=>$list));
    }
