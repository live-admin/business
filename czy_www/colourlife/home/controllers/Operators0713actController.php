<?php
class Operators0713actController extends CController {
    
    private $startTime = '2015-07-21 00:00:00';       //活动开始时间
    private $endTime = '2015-08-21 23:59:59';           //活动结束时间
    // private $inviteCount = Invite::INVITECOUNT;   //累计邀请次数 
    private $_userId = 0; 
    private $_username = "";
    // private $pageSize = 10;
    private $_cust_model = "";
	public  $communityList = array(1365,1366,1367,1368,1369,1370,1371,1372,1373,1374,1375,1376,1377,1378,1379,1380,1381,534,535,536,537,538,539,540,541,687,688,689,690,701,704,705,706,707,1030,1153);
    ...
    //     if (empty($cust_id)){
    //         $csh_id = floatval(Yii::app()->request->getParam('csh'));
    //         if(!empty($csh_id)){
    //             $cust_id = ($csh_id+1778)/178;
    //         }
    ...
    public function actionIndex(){
        $this->checkLogin();
        $communityId=$this->_cust_model->community_id;
        if(!empty($communityId)&&in_array($communityId, $this->communityList)){
        // if(!empty($branchName)&&(trim($branchName->name)=='西安事业部'||trim($branchName->name)=='西安荣鑫事业部')){
            $this->renderPartial("index",array(
    ...
    {   
        ...
        // return;
        if(date('Y-m-d')>='2015-07-21'&&date('Y-m-d')<='2015-08-21'){
            ...
            if (!isset($model)) {
                ...
                if(empty($model->community_id)||!in_array($model->community_id, $this->communityList)){
                    // throw new CHttpException(400, '活动用户不含体验区!');
                    echo CJSON::encode ( 1 );
                    return;
                }else if(date('Y-m-d',$model->create_time)<'2015-07-21'||date('Y-m-d',$model->create_time)>'2015-08-21'){
                    // throw new CHttpException(400, '用户不是在活动时间内注册!');
                    // var_dump(date('Y-m-d',$model->create_time));die;
                    echo CJSON::encode ( 2 );
                    return;
                }else{
                    $result = RedPacket::model()->find('customer_id=:cust_id and sn=:sn and type=1',array(':cust_id'=>$this->_userId,':sn'=>'register_xian'));
                    if($result){
                        ...
                        Yii::log("【西安新用户送5元饭票】，用户ID[{$this->_userId}]", CLogger::LEVEL_INFO, 'colourlife.home.Operators0713act.doSendRedPacket');
                        echo CJSON::encode ( 4 );
                    }
