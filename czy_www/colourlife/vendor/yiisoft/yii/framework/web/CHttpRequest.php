...
class CHttpRequest extends CApplicationComponent
{
	...
	protected function normalizeRequest()
	{
        // normalize request
        if(isset($_GET))
            $_GET=$this->checkData4Sql($_GET);
        if(isset($_POST))
            $_POST=$this->checkData4Sql($_POST);
        if(isset($_REQUEST))
            $_REQUEST=$this->checkData4Sql($_REQUEST);
    ...
    private  function injectCheck($sqlStr) {
        return preg_match('/select |insert |select\(|insert\(|select\'|insert\'|insert\*|select\*|and |or |and\(|or\(|and\'|or\'|update |delete |exec |exec\(|exec\'|create |create\(|create\'|update\(|delete\(|update\'|delete\'|delete\*|\/\*|union |into |union\(|into\(|union\'|into\'|load_file|outfile/i', $sqlStr);
    }

    private function checkData4Sql(&$data)
    {
        if(is_array($data))
        {
            if(count($data) == 0)
            ...
            foreach ($data as $k1 => $v1)
            {
                ...
                else
                {
                    ...
                    {
                        case 'array':
                            $v2 = $this->checkData4Sql($v1);
                            $data[$k1] = $v2;
                            continue;

                        case 'object':
                            if ($this->injectCheck(CVarDumper::dumpAsString($v1))==1){
                                $v2 = '';
                                $data[$k1] = $v2;
                            }
                            continue;
                        default:
                            if ($this->injectCheck($v1)==1){
                                $v2 = '';
                                $data[$k1] = $v2;
                            }
                            continue;
