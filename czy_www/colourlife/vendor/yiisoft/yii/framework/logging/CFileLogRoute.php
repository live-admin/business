...
class CFileLogRoute extends CLogRoute
{
	...
	protected function processLogs($logs)
	{
		...
		foreach($logs as $log) {
			...
			//是否为colourlife.logFile记录在文本日志类型，是则重新赋值修改文件名称为colourlife.logFile后的第一个单词
			if(strpos($log[2],'colourlife.logFile')!== false){
				$catogry = explode(".", $log[2]);
				$fileName = $catogry[2].'.log';
			}
		}

		//$logFile=$this->getLogPath().DIRECTORY_SEPARATOR.$this->getLogFile();
		$logFile=$this->getLogPath().DIRECTORY_SEPARATOR.$fileName;
