...
class PayService extends BaseService{
	 ...
	 * 通过商户号获取合并后的支付方式
	 */
	public function getNewPayListByBusinessId($businessId,$customerId,$isOnlyInfo = false){
		if (empty($customerId)){
		...
		Yii::log("调用商圈的支付列表开始",CLogger::LEVEL_ERROR,'colourlife.core.PayService.getPayListByBusinessId');
		$bidPayList = $this->getNewBusinessPayList($businessId, $customerId);
		
		if (empty($bidPayList) || empty($bidPayList['pay_list'])){
		...
		if ($isOnlyInfo){
			return array(
						'id' => $bidPayList['pay_info']['shop_id'],
						'name' => $bidPayList['pay_info']['name'],
						'finance_mobile' => $bidPayList['pay_info']['finance_mobile'],
						'business_uuid' => $bidPayList['pay_info']['business_uuid']
					);
		}
		//获取用户的支付方式
		$payList = $this->mergePayList($bidPayList['pay_list'],$customerId);
		$data = array(
				'info' =>array(
					'id' => $bidPayList['pay_info']['shop_id'],
					'name' => $bidPayList['pay_info']['name'],
	 ...
	 * 2017-08-10
	 */
	public function getNewBusinessPayList($businessId,$customerId){
		if (empty($businessId)){
		...
		$microService = new MicroBusinessService();
		$result = $microService->dispatchService(
			'api/pay/list',
			$param
		);
		if(!$result)
...
//		}
	...
	public function microBusinessCallBack($customer_id , $shop_id , $third_sn , $colour_sn , $amount , $payment_id , $payment_type , $pay_time)
	{
		if (empty($customer_id)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,用户ID为空', '新交易回调,用户ID为空');
		}
		if (empty($shop_id)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,店铺为空', '新交易回调,店铺为空');
		}
		if (empty($third_sn)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,三方订单为空', '新交易回调,三方订单为空');
		}
		if (empty($colour_sn)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,彩之云订单为空', '新交易回调,彩之云订单为空');
		}
		if (empty($amount)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,金额为空', '新交易回调,金额为空');
		}
		if (empty($payment_id)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,支付方式为空', '新交易回调,支付方式为空');
		}
		if (empty($payment_type)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '2新交易回调,支付方式2为空', '新交易回调,支付方式2为空');
		}
		...
		$param = array(
			...
			'ts' => $pay_time,
			'sign' => md5($customer_id.$shop_id.$amount.'colourlife'.$pay_time)
		);
		Yii::log(
			sprintf(
				'调用微商圈接口哦哦哦: %s.',
...
// 		dump($post);
	 ...
	 * 2017-08-10新微商圈
	 */
	public function getNewPaymentInfo($businessId = 0,$customerId,$pano,$sn = ''){
		if (empty($customerId)){
			return $this->sendErrorMessage(false, ErrorCode::PARAM_VALUE_EMPTY, '获取单个支付方式信息,用户ID不能为空', '获取单个支付方式信息,用户ID不能为空');
		}
		if (empty($pano)){
		...
		if (!empty($businessId)){
			$payList = $this->getNewPayListByBusinessId($businessId,$customerId);
		}else {
			$payList = $this->getPayListBySn($sn, $customerId);
		}
		if (!isset($payList['list'][$pano])){
	...
	public function prepayForColor($money,$content = '',$orgtype = 0,$orgaccountno,$desttype = 0,$destaccountno,$detail = '',$starttime = 0,$stoptime = 0, $callback = '',$orderno = '',$remoteip = '', $customerId = ''){
		...
		try {
			Yii::log('调用金融平台消费接口开始：',CLogger::LEVEL_INFO, 'colourlife.core.PayService.prepay');
			$result = FinanceMicroService::getInstance()->prepayForColor(
				$money,  //交易金额
				$content,  //备注
				intval($orgtype), //支付类型
				$orgaccountno, //支付账号
