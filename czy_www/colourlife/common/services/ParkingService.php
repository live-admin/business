...
class ParkingService extends BaseService {
     ...
     */
    public function getMonthCategory($communityId, $carNumber,$mobile = 0)
    {
        $parkingTypes = array();
        $feeCategory = Item::PARKING_TYPE_COLOURLIFE;
        $thirdFees = '';

        $isStation = $this->isStation($communityId);
        if ($isStation && isset($isStation['url'])){
        	//接入易停车
        	$feeCategory = Item::PARKING_TYPE_YITINGCHE;
        	$parkingFees = $this->getEParkingTypes($isStation['uuid'], $carNumber,$isStation['stations']);
        	if ( empty($parkingFees) )
        		throw new CHttpException(400, "不是e停车缴费停车场月卡用户");
        	$parkingTypes = $parkingFees['fee_unit_list'];
        	$thirdFees = json_encode(array('uuid' => $isStation['uuid'],'mobile' => $mobile,'parkingFees' => $parkingFees['data']));
        	return array(
    ...
    public function makeParkingData($carNumber, $parkingType, $feeUnitId, $feeNumber, $thirdFees, $orderSn)
    {
        ...
        elseif (Item::PARKING_TYPE_YITINGCHE === $parkingType) {  //易停车
        	$amount = F::priceFormat($thirdFees['parkingFees'][$feeUnitId]['money'] * $feeNumber);
        	Yii::import('common.api.YitingcheApi');
        	$result = YitingcheApi::getInstance()->createEparkingOrder($orderSn, $amount, $thirdFees['uuid'], $carNumber, $feeNumber, $thirdFees['mobile'],$feeUnitId);
        	if (false === $result)
        		throw new CHttpException(400, '请求易停车订单返回失败');
        
        	$parkingData['third_park_type'] = Item::PARKING_TYPE_YITINGCHE;
        
        	$parkingData['fee_number'] = $feeNumber;
        	$parkingData['fee_unit'] = $thirdFees['parkingFees'][$feeUnitId]['money'] . '/月';
        
        	$parkingData['third_park_id'] = $thirdFees['uuid'];
        	$parkingData['third_order_id'] = $result['tnum'];
        	$parkingData['out_result'] = json_encode($result);
    ...
    public function monthTransactionSuccess(OthersFees $otherFeeModel)
    {
        ...
        // 易停车
        elseif (intval($parkingFees->third_park_type) === Item::PARKING_TYPE_YITINGCHE) {
        	Yii::import('common.api.YitingcheApi');
        	$result = YitingcheApi::getInstance()->notify($parkingFees->third_order_id, $otherFeeModel->sn);
     ...
     * @return boolean
     */
    public function isStation($community_id){
    	if (empty($community_id)){
    		return $this->sendErrorMessage(false, 0, '小区ID不能为空', '小区ID不能为空');
    	}
    	Yii::import('common.api.IceApi');
    	//获取小区信息
    	$community = IceApi::getInstance()->getCommunityInfo($community_id);
    	if (false === $community){
    	 	return $this->sendErrorMessage(false, 0, '请求ICE返回失败', '请求ICE返回失败');
    	}
    	if (!isset($community['uuid']) || empty($community['uuid'])){
    		return $this->sendErrorMessage(false, 0, 'uuid不能为空', 'uuid不能为空');
    	}
    	Yii::import('common.api.YitingcheApi');
    	try {
    		$result = YitingcheApi::getInstance()->getStation($community['uuid']);
    		if (!$result){
    			return $this->sendErrorMessage(false, 0, '无数据返回', '无数据返回');
    		}
    		$info = HomeConfigResource::model()->getResourceByKeyOrId('ETingCheH5');
    		if ($info->completeURL)
    			return array(
    		...
    		//dump($e->getMessage());
    		return $this->sendErrorMessage(false, 0, '判断是否是易停车的小区调用易停车api异常：'.$e->getMessage(), '判断是否是易停车的小区调用易停车api异常:'.$e->getMessage());
    	}
    }
     ...
     */
    public function getEParkingTypes($uuid,$carnum,$stations){
    	if (empty($uuid) || empty($carnum) || empty($stations)){
    		return $this->sendErrorMessage(false, 0, '参数不能为空', '参数不能为空');
    	}
    	$data = array();
    	Yii::import('common.api.YitingcheApi');
    	try {
    		foreach ($stations as $val){
    			$parkingFees = YitingcheApi::getInstance()->getMonthDetails($uuid, $carnum,$val['bid']);
    			if (empty($parkingFees)){
    				continue;
    			}
    			$data['fee_unit_list'][] = array(
    						'id' => $val['bid'],
    						'name' => '月',
    						'fees' => $parkingFees[0]['money'],
    						'unit_name' => '个月',
    						'unit' => array(1,2,3,4,6,12)
    				);
    			$data['data'][$val['bid']] = $parkingFees[0];
    		}
    		return $data;
    	}catch (Exception $e){
    		return $this->sendErrorMessage(false, 0, '调用接口异常：'.$e->getMessage(), '调用接口异常：'.$e->getMessage());
    	}
    }
