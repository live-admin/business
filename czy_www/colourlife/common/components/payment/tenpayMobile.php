 ...
 * 财付通手机插件
 */
class tenpayMobile extends PayFactory
{
    ...
    public function get_code($pay, $payment)
    {
        ...
        /* 返回的路径 */
        $return_url = PayLib::return_url('tenpayMobile');
        $notify_url = PayLib::notify_url('tenpayMobile');
        // 商户号
    ...
    public function respond($type = 'return')
    {
        ...
        $payment = PayLib::get_payment('tenpayMobile');
        ...
        if ($resHandler->isTenpaySign()) {
            ...
            if ("0" == $pay_result) {
                ...
                if (!PayLib::check_money($this->_sn, $amount)) {
                    Yii::log('财付通手机流水号' . $this->_sn .'检查支付的金额不相符', CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayMobile');
                    return false;
                }
                ...
                if ($type == 'notify' && Pay::getPayStatus($this->_sn) == 0) { //状态为0才能去修改状态
                    ...
                    if(!PayLog::createPayLog($this->_sn, $amount, $payment->id))
                        Yii::log('财付通手机添加支付记录失败流水号为：' . $this->_sn .' 支付金额：' . $amount, CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayMobile');
                    else
                    {
                        /* 改变订单状态 */
                        PayLib::order_paid($this->_sn, $payment->id);
                        Yii::log('财付通手机成功支付流水号为：' . $this->_sn .' 支付金额：' . $amount, CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayMobile');
                    }
                    return true;
                }
            } else {
                Yii::log('财付通手机支付失败流水号为：' . $this->_sn, CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayMobile');
                return false;
            }

        } else {
            //回调签名错误
            Yii::log('回调签名错误', CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayMobile');
            return false;
        }
    }
