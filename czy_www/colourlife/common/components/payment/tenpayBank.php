 ...
 * 财付通手机插件
 */
class tenpayBank extends PayFactory
{
    public $returnUrl;
    ...
    {
        //默认返回地址
        $this->returnUrl = F::getOrderUrl('/wap/tenpayBank');
    }
    ...
    public function get_code($pay, $payment)
    {
        ...
        $notify_url = PayLib::notify_url('tenpayBank');
        // 商户号
        ...
        Yii::log('财付通支付送出参数' . $note, CLogger::LEVEL_INFO, 'colourlife.core.payment.get_code.tenpayBank');

        //后台调用
        if ($httpClient->call()) {

            $resHandler->setContent($httpClient->getResContent());
            if (isset($resHandler->parameters['err_info']) && $resHandler->parameters['err_info'] != '' ) {
    ...
    public function respond($type = 'return')
    {
        ...
        $payment = PayLib::get_payment('tenpayBank');
        ...
        if ($resHandler->isTenpaySign()) {
            ...
            //————————————————————caohui 2015-02-11修改——————————————————————————————
            $payModel = Pay::getPayModel($this->_sn);

            $this->_id = $payModel->id;
            //————————————————————caohui 2015-02-11修改——————————————————————————————
            ...
            if ("0" == $pay_result) {
                ...
                if (!PayLib::check_money($this->_sn, $amount)) {
                    Yii::log('财付通银行流水号' . $this->_sn .'检查支付的金额不相符', CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayBank');
                    return false;
                }
                ...
                if ($type == 'notify' && Pay::getPayStatus($this->_sn) == 0) { //状态为0才能去修改状态
                    ...
                    if(!PayLog::createPayLog($this->_sn, $amount, $payment->id))
                        Yii::log('财付通银行添加支付记录失败流水号为：' . $this->_sn .' 支付金额：' . $amount, CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayBank');
                    else
                    {
                        /* 改变订单状态 */
                        PayLib::order_paid($this->_sn, $payment->id);
                        Yii::log('财付通银行成功支付流水号为：' . $this->_sn .' 支付金额：' . $amount, CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayBank');
                    }
                }
                return true;
            } else {
                Yii::log('财付通银行支付失败流水号为：' . $this->_sn, CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayBank');
                return false;
            }
        } else {
            //回调签名错误
            Yii::log('财付通银行回调签名错误', CLogger::LEVEL_INFO, 'colourlife.core.payment.tenpayBank');
            return false;
        }
    }
