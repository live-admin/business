 ...
 * 支付宝支付插件
 */
class alipay extends PayFactory
{
    public $returnUrl;
    ...
    public function get_code($pay, $payment)
    {
       ...
       // require_once($lib_path . 'alipay.config.php');
        ...
        $newlhqpayArr =$newlhqpay->get_code($pay, 'ALIPAY', Yii::app()->user->id);
        ...
        $linkString = $alipayCore->createLinkstring($array);
        $alipayRsa = new AlipayRsa();
        $sign = $alipayRsa->rsaSign($linkString,  $this->alipay_config['private_key_path']);
        $sign = urlencode($sign);
        $url = $linkString . '&sign="' . $sign. '"'  . '&sign_type="RSA"' ;
        $note = '金额：' . $total_fee . '返回url：' . $return_url . '支付流水号：' . $out_trade_no . '回调URL：' . $notify_url;
        Yii::log('支付宝支付送出参数' . $note, CLogger::LEVEL_INFO, 'colourlife.core.payment.get_code.alipay');
        return $url;
    }
    ...
    public function respond($type = 'return')
    {
        ...
        if (empty($_POST['out_trade_no']) || empty($_POST['trade_no']) || empty($_POST['trade_status'])
                || empty($_POST['total_fee']) || empty($_POST['seller_id']) || empty($_POST['buyer_email'])) {
        ...
        if ($verify_result) {
             //支付宝返回值
            $alipay_total_fee = $_POST['total_fee'];
            $amount = $alipay_total_fee;
            $alipay_seller_id = $_POST['seller_id'];
