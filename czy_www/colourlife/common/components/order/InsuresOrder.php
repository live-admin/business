 ...
 * Time: 下午3:49
 */
class InsuresOrder extends OrderFactory
{
    public $order;
    ...
    public function init($order_id, $payment_id = 0, $note = '')
    {
        ...
        $this->order = InsureOrder::model()->findByPk($order_id);
    ...
    {
        $this->order = InsureOrder::model()->findByPk($this->order_id);
        $this->status = Item::INSURE_ORDER_PAYED;
        ...
        if ($this->order->update() && OthersFeesLog::createOtherFeesLog($this->order_id, $this->model, $this->status, $this->note,$this->order->customer_id)) {
            ...
            if (false === $result)
                Yii::log("记录保险订单付款流水号'{$this->order_sn}' 状态:已付款，保险出单失败", CLogger::LEVEL_INFO, 'colourlife.core.InsureOrder');

//            try {
//                IntegralEvent::fees($this->order);//付款成功积分
//            } catch (Exception $e) {
//                Yii::log("记录保险订单付款流水号'{$this->order_sn}' 状态:已付款。送积分异常。", CLogger::LEVEL_INFO, 'colourlife.core.InsureOrder');
//            }
    ...
    private function balanceFailed()
    {
        ...
        if ($this->order->update() && OthersFeesLog::createOtherFeesLog($this->order_id, $this->model, $this->status, '红包余额不足，交易失败',$this->order->customer_id)) {
            Yii::log("记录保险订单付款流水号'{$this->order_sn}' 红包余额不足，交易失败。", CLogger::LEVEL_INFO, 'colourlife.core.insureOrder');
            return true;
        }
        return false;
    }
    ...
    private function sendSms()
    {
        try {
            ...
            Yii::log("记录保险订单付款流水号'{$this->order_sn}' 状态:已付款。发短信异常。", CLogger::LEVEL_INFO, 'colourlife.core.insureOrder');
        }
    ...
    public function orderProcessing()
    {
        if ($this->order->red_packet_pay > 0  || (isset($this->order->pay_info) && !empty($this->order->pay_info))) { //使用红包抵扣
            ...
            if ($this->order->red_packet_pay <= $this->balance) { //余额可以支付
                ...
                $attr['to_type'] = Item::RED_PACKET_TO_TYPE_INSURE;
