 ...
 * @author    Ender
 */
class HomeConfigMicroService
{


    private $apiServer = '';
    private $privilegeServer = '';
    private $appKey = '01a2b3c4d5e6f7a8b9c0';
    private $appSecret = '1a2b3c4d5e6f7a8b9c0d1a2b3c4d5e6f';
    private $accessToken = '';

    public function __construct()
    {
        if (defined('YII_DEBUG') && YII_DEBUG == true) {
            $this->apiServer = 'http://home.czytest.colourlife.com/';
            $this->privilegeServer = 'http://neotest.kakatool.cn:8098/';
        } else {
            $this->apiServer = 'http://home.colourlife.com/';  //正式地址
    ...
    protected function getAccessTokenFromPrivilegeMicroService()
    {
        ...
        if (!$response || !isset($response['code'])) {
            throw new CHttpException(501, '获取首页权限失败');
        }
        ...
        if (!isset($response['content']['access_token'])
            ...
            throw new CHttpException(502, '获取首页权限失败');
        }
    ...
    {
        $config = Config::model()->find('`key`=:key', array(':key' => 'HomeConfigMicroService'));
        if ($config) {
            $data = $config->getVal();
            if ($data
    ...
    protected function saveAccessTokenToDB($accessToken = '', $expireIn = 0)
    {
        ...
        if ($config) {
            ...
            $config = new Config();
            $config->key = 'HomeConfigMicroService';
            $config->name = '首页微服务 Token';
            $config->type = 'HomeConfigMicroService';
            $config->update_time = time();
            $config->setVal($data);
        }
    ...
    public function dispatchService($interface = '', $postData = array())
    {
        ...
        if (!$response || !isset($response['code'])) {
            throw new CHttpException(500, '首页微服务请求失败');
        }
