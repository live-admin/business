 ...
 */
class SetableActivityPic extends CActiveRecord
{
    public $modelName = '首页活动图片';
    ...
    public function rules()
    {
		...
		return array(
            array('small_loans_id, bg, bname, sname, title, url, state, sort,  act, proto_android, proto_ios', 'required', 'on' => 'update,create'),
           // array('secret', 'safe', 'on' => 'update,create'),
            array('id, small_loans_id, bg, bname, sname, title,state, sort, proto_android, proto_ios, act', 'safe', 'on' => 'search'),
           // array('sl_key','unique','on'=>'update,create'),
            array('community_ids,bg,bname,sname,parameter,imgFile','safe', 'on' => 'create,update'),
        );
    ...
    public function getImgUrl()
    {
		...
		$url = $res->setAbleUploadImg($this->bg);
		return $url;
    }
   ...
   protected function beforeSave()
   {
        if (!empty($this->imgFile)) {
            $this->bg = Yii::app()->ajaxUploadImage->moveSave($this->imgFile, $this->bg);
        }
   ...
   public function getCommunityTreeData()
    {
        ...
        return $branch->getRegionCommunityRelation($this->id, 'SetableActivityPic', false);
    }
