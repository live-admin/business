 ...
 */
class CaiRedpacketStep2 extends CActiveRecord
{	
	public $modelName = '彩管家红包创建第二步';
	public $relation_branchId;
	public $step1_relation;
	...
	public function rules()
	{
		...
		return array(			
            ...
            array('step1_id, is_locked, create_userid, update_userid, state, is_deleted', 'numerical', 'integerOnly'=>true),
			...
			array('step1_id', 'checkRepeat', 'on' => 'createStep2'),
			array('red_packet, step1_relation', 'checkQuery', 'on' => 'createStep2'),
			// array('branch_ids', 'checkBranch', 'on' => 'createStep2'),			
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, step1_id, name, red_packet, get_redpacket, frozen_sum, is_locked, create_username, create_userid, create_date, update_username, update_userid, update_date, state, is_deleted', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'branch' => array(self::HAS_MANY, 'CairedpacketBranchRelationStep2', 'cai_redpacket_id'),
			'step1' => array(self::BELONGS_TO, 'CaiRedpacketStep1', 'step1_id'),
		);
	}
    ...
    {	
        if (!$this->hasErrors() && !empty($this->step1_id) && !empty($this->relation_branchId)) {
        	$model = CairedpacketBranchRelationStep2::model()->findAll('step1_id='.$this->step1_id);
        	if($model){
        		$list = array();
        		$str="";
    ...
    {	
        if (!$this->hasErrors() && !empty($this->red_packet) && !empty($this->step1_relation)) {
        	$model = CairedpacketBranchRelation::model()->findByPk($this->step1_relation);
        	if($model){
	...
	public function search()
	{
        ...
        $criteria->compare('step1_id',$this->step1_id);
