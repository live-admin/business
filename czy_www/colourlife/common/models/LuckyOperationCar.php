...
class LuckyOperationCar {
	...
	public function custDoOperation($custname, $custid, $activityid, $prizeid,$redInfo=false, $besideGennerZero = true, $besideid = array(0)) {
		...
		try {
			...
			//减去用户的红包金额
			if(intval($prizeid)==137){ 
            	//精美礼品
            	$customer = Customer::model ()->findByPk ( $custid );
            	$redPacket=new RedPacket();
                $attr['customer_id']=$custid;
                $attr['type'] = Item::RED_PACKET_TYPE_CONSUME;//设置属性为消费
                $attr['sum']=0.1;
                $attr['to_type']=Item::RED_PACKET_TO_TYPE_LOTTERY_MAY_CAR;
                $attr['sn']='thanks_consum';
                $attr['note']= "【五月幸福中国行抽奖】消费红包【{$attr['sum']}】元";
                $balance = ($customer->getBalance()-$attr['sum']);
                $redPacket->setAttributes($attr);
                $redPacket->validate();
                if(!$redPacket->save() or !Customer::model()->updateByPk($custid,array('balance'=>$balance)) ){
                   Yii::log("用户ID{$custid},五月幸福中国行抽奖,红包0.1元扣除失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
					$transaction->rollback ();
					return false;
                }
                $transaction->commit ();
			...
			if( !in_array($prizeid, $besideid)){
                ...
                if(intval($prizeid)==140){ 
                	//精美礼品
                	$redPacket=new RedPacket();
                    $attr['customer_id']=$custid;
                    $attr['type'] = Item::RED_PACKET_TYPE_CONSUME;//设置属性为消费
                    $attr['sum']=0.1;
                    $attr['to_type']=Item::RED_PACKET_TO_TYPE_ROB_GOOD_GIFT;
                    $attr['sn']='good_gift';
                    $attr['note']= "【五月幸福中国行抽奖获得精美礼品】消费红包【{$attr['sum']}】元";
                    $balance = ($customer->getBalance()-$attr['sum']);
                    $redPacket->setAttributes($attr);
                    $redPacket->validate();
                    if(!$redPacket->save() or !Customer::model()->updateByPk($custid,array('balance'=>$balance)) ){
                       Yii::log("用户ID{$custid},五月幸福中国行抽奖获得精美礼品,红包0.1元扣除失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
						$transaction->rollback ();
						return false;
                    }
    			...
    			//黑莓酒礼盒
                if(intval($prizeid)==139){
                	$onecode = OneYuanBuy::sendCodeLucky($custid,1624,0,'lucky',$custResult->id);
                	if(!$onecode){
                		Yii::log("用户ID{$custid},商品ID{$goods_id}(黑莓酒礼盒)的一元购码发放失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
						$transaction->rollback ();
						return false;
                	}
                    ...
                    $attr['type'] = Item::RED_PACKET_TYPE_CONSUME;//设置属性为消费
                    $attr['sum']=0.1;
                    $attr['to_type']=Item::RED_PACKET_TO_TYPE_ROB_HEI_MEI_JIU;
                    $attr['sn']='rob_heimeijiu';
                    $attr['note']= "【五月幸福中国行抽奖获得黑莓酒礼盒】消费红包【{$attr['sum']}】元";
                    $balance = ($customer->getBalance()-$attr['sum']);
                    $redPacket->setAttributes($attr);
                    $redPacket->validate();
                    if(!$redPacket->save() or !Customer::model()->updateByPk($custid,array('balance'=>$balance)) ){
                       Yii::log("用户ID{$custid},五月幸福中国行抽奖获得黑莓酒礼盒,红包0.1元扣除失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
						$transaction->rollback ();
						return false;
                    }
                	LuckyCustResult::model()->updateByPk($custResult->id, array('deal_state'=>2,'deal_userid'=>0,'deal_username'=>'auto','isgivered'=>1));
					$result ['luckyCustResult'] ['one_yuan_code'] = $onecode;
                }

               	//甜蜜红枣
                if(intval($prizeid)==141){                	
                	$onecode = OneYuanBuy::sendCodeLucky($custid,1617,0,$type='lucky',$custResult->id);
                	if(!$onecode){
                		Yii::log("用户ID{$custid},商品ID{$goods_id}(甜蜜红枣)的一元购码发放失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
						$transaction->rollback ();
						return false;
                	}
                	$redPacket=new RedPacket();
                    ...
                    $attr['type'] = Item::RED_PACKET_TYPE_CONSUME;//设置属性为消费
                    $attr['sum']=0.1;
                    $attr['to_type']=Item::RED_PACKET_TO_TYPE_ROB_TONIC_MAY;
                    $attr['sn']='tonic';
                    $attr['note']= "【五月幸福中国行抽奖获得甜蜜红枣】消费红包【{$attr['sum']}】元";
                    $balance = ($customer->getBalance()-$attr['sum']);
                    $redPacket->setAttributes($attr);
                    $redPacket->validate();
                    if(!$redPacket->save() or !Customer::model()->updateByPk($custid,array('balance'=>$balance)) ){
                       Yii::log("用户ID{$custid},五月幸福中国行抽奖获得甜蜜红枣,红包0.1元扣除失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
						$transaction->rollback ();
						return false;
                    }
                	LuckyCustResult::model()->updateByPk($custResult->id, array('deal_state'=>2,'deal_userid'=>0,'deal_username'=>'auto','isgivered'=>1));
                ...
                //begin汽车抽奖码
                $result ['luckyCustResult'] ['car_code'] ="";
                if(intval($prizeid)==138){
                	$carcode = LuckyMayCarOutcome::model()->rob($activityid,$custid);
                	if(isset($carcode['code'])){
                		LuckyCustResult::model()->updateByPk($custResult->id, array('deal_state'=>2,'deal_userid'=>0,'deal_username'=>'auto','isgivered'=>1));
						$result ['luckyCustResult'] ['car_code'] = $carcode['code'];
                	}else{
                		//0没抢到(没扣红包)；1抢到了；2抢光了(号码不够)；3红包金额不足 5没抢到(扣了红包)
                		Yii::log("用户ID{$custid},汽车抽奖码发放失败,时间{date(Y-m-d H:i:s)}",CLogger::LEVEL_ERROR);
                		$transaction->rollback ();
						return false;
                	}
            	}
...
//			//抽奖机会，不关联活动，只关联人，统一放到id为2的活动去。
	...
	public function doLucky($luckyActId,$luckyCustCan,$luckyTodayCan,$userName,$userId,$besideGennerZero = true, $besideid = array(0)){
		...
		if(in_array($branchId,Item::$lucky_tiyan_branch_ids)){
			$retObj = array (
				'id' => intval(Item::LUCKY_THANKS_ID_CAR),
				'prize_name' =>"",
				'all' => 0,
				'prize_level_name' => 0,
				'cust_result_id' => 0,
		...
		if(in_array($getObj['id'],array(138,139,141))){//本期中过黑莓酒、汽车码、甜蜜红枣不会再中
			...
			if($getRed==0){ //产生红包失败，转给予谢谢参与
				...
				if(intval($getObj['delay'])==0){
					...
					$items = array(
						...
						'sum' =>$getRed,//红包金额,
						'sn' => Item::LUCKY_ACT_ID_CAR,//订单号(from_type=1or2的情况下)或活动ID(from_type=3的情况下)
					);
		...
		if ( $change != false ) {
			...
			$retObj = array (
					...
					'lucky_one_code' => $change ['luckyCustResult'] ['one_yuan_code'],
					'lucky_car_code' => $change ['luckyCustResult'] ['car_code'],
