 ...
 */
class OthersParkingFees extends OthersFees
{
	/**
	 * @var string 模型名
	 */
	...
	public $objectModel = 'ParkingFees';
	public $pay_sn;
	...
	public function rules()
	{
		$array = array(
			...
			array(
				'region,communityIds,username,endTime,startTime,build_name,pay_sn,type_id,mobile,community_name,build_id,room,car_number,community_id, parking_card_number,customer_id',
				'safe',
				'on' => 'search'
			),
			//			ICE 搜索数据
	...
	{
		$model = $this->objectModel;
		return empty($this->$model->parking_card_number) ? '' : $this->$model->parking_card_number;
	}

	public function getParkingType()
	{
		$model = $this->objectModel;
		return empty($this->$model->TypeName) ? '' : $this->$model->TypeName;
	}

	public function getParkingPeriod()
	{
		$model = $this->objectModel;
		return empty($this->$model->period) ? '0' : $this->$model->period;
	}
