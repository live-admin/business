 ...
 */
class SetLitChi extends CActiveRecord{
    public $modelName = "抢荔枝活动设置";
    ...
    public function getThisActivity(){
        return SetLitChi::model()->find('state = 0 and start_time like "%'.date('Y-m-d H',time()).'%"');
    }
    ...
    public function getNextHour(){
        ...
        self::$act_time += 3600;
        $nextHour = SetLitChi::model()->find('state = 0 and start_time like "%'.date('Y-m-d H',time()+self::$act_time).'%"');
        if(!$nextHour){
    ...
    public function getAct($activity_id){
        ...
        if($activity_id  <= $lastId){
            ...
            $model = SetLitChi::model()->findByPk(self::$act_id);
            if($model){
                if($model->state == 1){
                    self::$act_id++;
