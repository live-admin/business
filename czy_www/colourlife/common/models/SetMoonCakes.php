 ...
 */
class SetMoonCakes extends CActiveRecord{
    public $modelName = "抢月饼活动";
    ...
    public function getThisActivity(){
        return SetMoonCakes::model()->find('state = 0 and start_time like "%'.date('Y-m-d H',time()).'%"');
    }
    

    //获取本场抢月饼活动
    public function getNewThisActivity(){
        return SetMoonCakes::model()->find('state = 0 and start_time <="'.date('Y-m-d H:i:s',time()).'" and end_time >="'.date('Y-m-d H:i:s',time()).'"');
    }
    ...
    public function getNextHour(){
        ...
        self::$act_time += 3600;
        $nextHour = SetMoonCakes::model()->find('state = 0 and start_time like "%'.date('Y-m-d H',time()+self::$act_time).'%"');
        if(!$nextHour){
    ...
    public function getAct($activity_id){
        ...
        if($activity_id  <= $lastId){
            ...
            $model = SetMoonCakes::model()->findByPk(self::$act_id);
            if($model){
                if($model->state == 1){
                    self::$act_id++;
