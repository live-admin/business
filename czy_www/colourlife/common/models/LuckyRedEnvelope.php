...
/**
 * This is the model class for table "lucky_red_envelope".
 *
 * The followings are the available columns in table 'lucky_red_envelope':
 * @property int $id
 ...
 */
class LuckyRedEnvelope extends CActiveRecord
{
    public $modelName = '红包奖项';
    ...
    public function relations() {
        return array(
            'luckyPrize' => array(self::BELONGS_TO, 'LuckyPrize', 'lucky_prize_id'),
        );
    }

    public function getPrizeName(){
        if($this->luckyPrize){
            return $this->luckyPrize->prize_name;
        }else{
            return "";
        }
    }
    ...
    public function check_total_number(){
       if($this->parent_id == 0){
            $redEnvelopes = LuckyRedEnvelope::model()->findAll('lucky_prize_id=:lucky_prize_id and parent_id=0',array(':lucky_prize_id'=>$this->lucky_prize_id));
            $prize = LuckyPrize::model()->findByPk($this->lucky_prize_id);
            $last = $prize->prize_count_all;
	...
	public function gennerRedPackage($userId,$redId){
		...
		//
		//查询该红包信息
		$sql2="SELECT * FROM `lucky_red_envelope` WHERE id=".$redId;
		...
		//例举该红包分配的所有地市
		$sql3="SELECT id,last_total,city_id FROM `lucky_red_envelope` WHERE `parent_id`=".$redId;
		...
		if(empty($redSons)){//如果没有分配地市限制
			...
			if($getRed <= $redInfo['last_total']){ //当前产生的红包，不比剩余大，则为有效
				$sql4="UPDATE lucky_red_envelope SET last_total=last_total-".$getRed." WHERE id=".$redId;
			...
			//当前用户所在的地市是否有限制
// 			$sql5="SELECT * FROM lucky_red_envelope WHERE `city_id`=".$cityId;
...
// 			$cityRed=$comm5->queryRow();
			...
			if(empty($myCityRed)){  //当前用户在的地市没有限制红包 
				...
				if($getRed <= $all){ //当前产生的红包，不比剩余大，则为有效
					$sql6="UPDATE lucky_red_envelope SET last_total=last_total-".$getRed." WHERE id=".$redId;
				...
				if($getRed <= $myCityRed['last_total']){ //当前产生的红包，不比剩余大，则为有效
					//当前记录和父记录，都减去相应的值
					$sql7="UPDATE lucky_red_envelope SET last_total=last_total-".$getRed." WHERE id=".$redId." OR id=".$myCityRed['id'];
	...
	public function gennerRedPackage2($userId,$redId){
		...
		$sql1="SELECT id,surplus_number FROM `lucky_red_envelope` WHERE id=".$redId;
		...
		$sql2="SELECT id,surplus_number,money  FROM `lucky_red_envelope` WHERE `parent_id`=".$redId;
		$comm2=$conn->createCommand($sql2);
		...
		$sql3="SELECT id,surplus_number,city_id FROM `lucky_red_envelope` WHERE `parent_id`=".$redChoose['id'];
		$comm3=$conn->createCommand($sql3);
		...
		if(empty($redChooseSons)){//如果没有分配地市限制
			...
			//直接产生红包并处理
			//红包箱-1，红包箱里的该红包-1
			$sql4="UPDATE lucky_red_envelope SET surplus_number=surplus_number-1 WHERE id=".$redId." OR id =".$redChoose['id'];
			$comm4=$conn->createCommand($sql4);
			if($comm4->execute()){  //更新该红包成功，返回本次得到的结果值
			...
			if(empty($myCityRed)){  //当前用户在的地市没有限制红包
				...
				if($all>0){ //还有数量
					$sql6="UPDATE lucky_red_envelope SET surplus_number=surplus_number-1 WHERE id=".$redId." OR id=".$redChoose['id'];
					$comm6=$conn->createCommand($sql6);
					if($comm6->execute()){  //更新该红包成功，返回本次得到的结果值
						return ($redChoose['money']);
					}
				...
				if($myCityRed['surplus_number']>0){ //还有数量
					...
					$sql7="UPDATE lucky_red_envelope SET surplus_number=surplus_number-1 WHERE id=".$redId." OR id=".$redChoose['id']." OR id=".$myCityRed['id'];
					$comm7=$conn->createCommand($sql7);
					if($comm7->execute()){  //更新该红包成功，返回本次得到的结果值
						return ($redChoose['money']);
					}
	...
	public function gennerRedPackage3($userId,$prizeid){
		...
		$sql2="SELECT id,surplus_number,money FROM `lucky_red_envelope` WHERE lucky_prize_id=".$prizeid ." AND parent_id=0";
		$comm2=$conn->createCommand($sql2);
		$redSons=$comm2->queryAll();
		...
		if(empty($redChooseSons)){//如果没有分配地市限制
			...
			//直接产生红包并处理
			//红包-1
			$sql4="UPDATE lucky_red_envelope SET surplus_number=surplus_number-1 WHERE id =".$redChoose['id'];
			$comm4=$conn->createCommand($sql4);
			if($comm4->execute()){  //更新该红包成功，返回本次得到的结果值
				return ($redChoose['money']);
			}
			...
			if(empty($myCityRed)){  //当前用户在的地市没有限制红包
				...
				if($all>0){ //还有数量
					$sql6="UPDATE lucky_red_envelope SET surplus_number=surplus_number-1 WHERE id=".$redChoose['id'];
					$comm6=$conn->createCommand($sql6);
					if($comm6->execute()){  //更新该红包成功，返回本次得到的结果值
						return ($redChoose['money']);
					}
				...
				if($myCityRed['surplus_number']>0){ //还有数量
					//当前记录和父记录，都减去相应的值
					$sql7="UPDATE lucky_red_envelope SET surplus_number=surplus_number-1 WHERE id=".$redChoose['id']." OR id=".$myCityRed['id'];
					$comm7=$conn->createCommand($sql7);
					if($comm7->execute()){  //更新该红包成功，返回本次得到的结果值
						return ($redChoose['money']);
					}
