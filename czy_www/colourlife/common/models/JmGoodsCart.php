...
/**
 * This is the model class for table "jm_goods_cart".
 *
 ...
 */
class JmGoodsCart extends CActiveRecord
{
    /**
     * @var string 模型名
     */
    public $modelName = '加盟商品购物车';
    public $debug = true;
    ...
    public function rules()
    {
        return array(
            ...
            array('good_id, number, buyer_id, shop_id, create_time', 'numerical', 'integerOnly'=>true),
            array('good_name', 'length', 'max'=>200),
            array('good_price', 'length', 'max'=>10),
            array('id, good_id, number, good_name, good_price, shop_id, buyer_id, create_time', 'safe', 'on'=>'search'),
        );
    }
    ...
    /**
     * 加入购物车功能
     * @param	array(goods_id,number,good_price,good_name,buyer_id)	一维或多维数组,必须包含键值名:
     * @return 	bool
     */
    public function addCart($items = array()) {
        ...
        if(isset($items['good_id'])) {
            $cgCart = JmGoodsCart::model()->findByAttributes(
                array('buyer_id'=>$buyer_id,'good_id'=>$items['good_id']));
            ...
            foreach($items as $val) {
                if(is_array($val) AND isset($val['good_id'])) {
                    $cgCart = JmGoodsCart::model()->findByAttributes(
                        array('buyer_id'=>$buyer_id,'good_id'=>$val['good_id']));
                    //如果商品不存在。那么插入购物车，否则数目增加传入数目
                    if(empty($cgCart)){
    ...
    public function total() {
        $cart_contents = JmGoodsCart::model()->findAllByAttributes(array('buyer_id'=>Yii::app()->user->id));
        $cart_total = 0;
        foreach($cart_contents as $cart){
            $cart_total += ($cart['good_price'] * $cart['number']);
        }
    ...
    public function contents() {
        $cart_contents = array();
        $cgCart = JmGoodsCart::model()->findAllByAttributes(
    ...
    private function _log($msg) {
        //echo $msg;exit;
        Yii::log($msg,CLogger::LEVEL_INFO, 'colourlife.shopBackend.JmGoodsCart.log');
    }
    ...
    public function SplitCartByShop(){
        ...
        foreach($cartList as $shop_id=>$cartArr){
            ...
            //查询商家的加盟关系
            $relation = ShopRelation::model()->findByAttributes(
                array('supplier_id'=>$shop_id,'seller_id'=>$buyer_id));
            if(empty($relation)){
                $return['error'] = "购物车存在异常商家(买家未加盟商家)，无法结算！";
                return $return;
            };
