...
{	
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('id, sn, employee_id, bind_bank_id, bank_id, card_holder, card_num, ide_num,red_packet, real_money, rate, create_time, status, update_username, update_userid, update_time, note, remake, startTime, endTime, bankName, employeeName, OAName, employeeMobile', 'safe', 'on'=>'search'),
            array('id, sn, employee_id, bind_bank_id, bank_id, card_holder, card_num, ide_num,red_packet, real_money, rate, create_time, status, update_username, update_userid, update_time, note, remake, startTime, endTime, bankName, employeeName, OAName, employeeMobile', 'safe', 'on'=>'report_search'),
        );
    }
    ...
    public function search()
    {
        ...
        if ($this->ide_num!= '') {
            $criteria->with[] = 'bindbank';
            $criteria->compare("bindbank.ide_num", $this->ide_num);
        }
...
//      ICE 上面三个搜索就是为了让查询更准确，而不是用表里面的什么，所以按照以下接入ice
    ...
    public function getIdeNum()
    {
        return empty($this->bindbank)?'':$this->bindbank->ide_num;
    }
