 ...
 * @version 母亲节model
 */
class MuQin extends CActiveRecord{
    private $beginTime='2017-05-11 00:00:00';//活动开始时间
	private $endTime='2017-05-21 23:59:59';//活动结束时间
    private $activityName='muqin';
    private $chaceDay=3;//每天抽奖次数
    private $you_hui_quan_id=array(100000591,100000590);
    //抽奖奖品配置
    ...
    public function getCoupon($customerId){
        ...
        $checkCoupons=UserCoupons::model()->find('mobile=:mobile and you_hui_quan_id=:you_hui_quan_id',array('mobile'=>$mobile,'you_hui_quan_id'=>$this->you_hui_quan_id[0]));
        if(empty($checkCoupons)){
            $str='';
            foreach ($this->you_hui_quan_id as $k=>$v){
                $str .= "('" .$mobile."',".$v.",0,0,".time()."),";
            }
            $str = trim($str, ',');
            $sqlInsert = "insert into user_coupons(mobile,you_hui_quan_id,is_use,num,create_time) values {$str}";
    ...
    private function checkAll($customer_id){
        ...
        $n1=MuqinPrize::model()->count('prize_id=:prize_id and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>1,':beginTime'=>$beginTime,':endTime'=>$endTime));
        $n2=MuqinPrize::model()->count('prize_id=:prize_id and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>2,':beginTime'=>$beginTime,':endTime'=>$endTime));
        $n3=MuqinPrize::model()->count('prize_id=:prize_id and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>3,':beginTime'=>$beginTime,':endTime'=>$endTime));
        $n4=MuqinPrize::model()->count('prize_id=:prize_id and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>4,':beginTime'=>$beginTime,':endTime'=>$endTime));
        //优惠券判断
        $check1=MuqinPrize::model()->find('prize_id=:prize_id and mobile=:mobile and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>1,':mobile'=>$mobile,':beginTime'=>$beginTime,':endTime'=>$endTime));
        $check2=MuqinPrize::model()->find('prize_id=:prize_id and mobile=:mobile and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>2,':mobile'=>$mobile,':beginTime'=>$beginTime,':endTime'=>$endTime));
        $check3=MuqinPrize::model()->find('prize_id=:prize_id and mobile=:mobile and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>3,':mobile'=>$mobile,':beginTime'=>$beginTime,':endTime'=>$endTime));
        $check4=MuqinPrize::model()->find('prize_id=:prize_id and mobile=:mobile and create_time>=:beginTime and create_time<:endTime',array(':prize_id'=>4,':mobile'=>$mobile,':beginTime'=>$beginTime,':endTime'=>$endTime));
    ...
    public function getChanceValue($customer_id){
    	...
    	$endTime = mktime(23, 59, 59);
        $mobile=$this->getMobileByCustomerId($customer_id);
        $userChance=MuqinPrize::model()->count('mobile=:mobile and create_time>=:beginTime and create_time<:endTime',array(':mobile'=>$mobile,':beginTime'=>$beginTime,':endTime'=>$endTime));
        if(empty($userChance)){
    ...
    private function insertPrize($customer_id,$prize_id,$prize_name){
        ...
        $mobile=$this->getMobileByCustomerId($customer_id);
        $muPriceModel=new MuqinPrize();
        $muPriceModel->mobile=$mobile;
        $muPriceModel->prize_id=$prize_id;
        $muPriceModel->prize_name=$prize_name;
        $muPriceModel->create_time= time();
        $isInsert=$muPriceModel->save();
        if($isInsert){
            return true;
        }else{
            return false;
    ...
    public function getPrizeDetail($customer_id){
        ...
        $mobile=$this->getMobileByCustomerId($customer_id);
        $prizeMobileArr = MuqinPrize::model()->findAll(array(
