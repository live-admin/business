...
//        return ($this->type==0)?(empty($this->receiver)?'':$this->receiver->mobile):(empty($this->customer)?'':$this->customer->mobile);
    ...
    public static function createRedPacketCarryOrder($feeAttr)
    {
        ...
        //把对应的邀请记录设置成已发送红包
        try{
            Yii::log("红包转账1".json_encode($items_to).">>>>".json_encode($items_from),CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
        	if(!$other->validate()||!$other->save()){
        		Yii::log("红包转账2验证失败或保存失败".json_encode($items_to).">>>>".json_encode($items_from),CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
	        	CaiRedpacketCarry::model()->addError('id', "红包转账失败！");
	        	return false;
        	}
        	...
        	if(!$ret_to){
            	Yii::log("红包转账失败,ret_to结果：".$ret_to.'参数列表:'.json_encode($items_to).">>>>".json_encode($items_from),CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
            	//@$other->delete();
        	...
        	if(!$ret_from){
            	Yii::log("红包转账失败,ret_from结果：".$ret_from.'参数列表:'.json_encode($items_to).">>>>".json_encode($items_from),CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
            	//@$other->delete();
            	$transaction->rollback();
            	return false;
        	}
            ...
            if($result){
                Yii::log("红包转账更新cai_redpacket_carry成功，结果：".$result,CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
                // 发起交易
                ...
                if(!$ftransaction || !isset($ftransaction['payinfo']) || !isset($ftransaction['payinfo']['tno'])){
                    Yii::log("红包交易失败,ftransaction结果：".json_encode($ftransaction).'记录ID:'.$other->id,CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
            	...
            	Yii::log("红包转账更新cai_redpacket_carry失败,参数列表".json_encode($items_to).">>>>".json_encode($items_from),CLogger::LEVEL_ERROR,'colourlife.core.caiRedpacketCarry.redPacketCarryOrder');
            	//@$other->delete();
            	$transaction->rollback();
            	return false;
            }
    ...
    private static function transaction($items_to, $items_from, $note){
        ...
        if(isset($items_to['to_type']) && $items_to['to_type'] == Item::CAI_RED_PACKET_TO_TYPE_COLOURLIFE){
            ...
            $destAccount = FinanceCustomerRelateModel::model()->find('customer_id=:customer_id and pano=:pano and atid=:atid',array(':customer_id'=>$items_from['customer_id'],':pano'=>$pano,':atid'=>$destatid));
        } else {
            // OA内部转账
            $destatid = $orgatid;
