...
/**
 * This is the model class for table "essay_category".
 *
 * The followings are the available columns in table 'essay_category':
 * @property integer $id
 ...
 */
class EssayCategory extends CActiveRecord
{
    /**
     * @var 标记是否搜索全部数据
     */
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('name, logo, desc', 'length', 'max' => 255),
            array('is_deleted', 'length', 'max' => 50),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, name, parent_id, logo, desc, state, is_deleted', 'safe', 'on' => 'search'),
    ...
    public function relations()
    {
        ...
        return array(
            'parent' => array(self::BELONGS_TO, 'EssayCategory', 'parent_id'),
            'enabledEssayCount' => array(self::STAT, 'Essay', 'category_id', 'condition' => 't.is_deleted=0'),
        );
    }
    ...
    public function checkDisable($attribute, $params)
    {
        ...
        // 需要判断是否有文章
        if (!$this->hasErrors() && !empty($this->enabledEssayCount)) {
            $this->addError($attribute, '因为该' . $this->modelName . '下还存在文章，无法禁用。');
        }
    ...
    public function checkDelete($attribute, $params)
    {
        ...
        // 需要判断是否有物业用户
        if (!$this->hasErrors() && !empty($this->enabledEssayCount)) {
            $this->addError($attribute, '因为该' . $this->modelName . '下还存在文章，无法删除。');
        }
    ...
    {
        $essay = self::model()->find('name=:name', array(':name' => $this->name));
        if (!$this->hasErrors() && !empty($essay)) {
