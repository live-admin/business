 ...
 */
class OthersPropertyFees extends OthersFees
{
	/**
	 * @var string 模型名
	 */
	...
	{
		$model = $this->objectModel;
		return empty($this->$model->colorcloud_order) ? '' : $this->$model->colorcloud_order;
	}
	...
	public static function createPropertyOrder($feeAttr, $propertyAttr)
	{
		...
		if (!isset($result) || $result['code'] != 0) {
			Yii::log('彩之云订单创建失败！返回信息：' . $result['message'], CLogger::LEVEL_INFO,
				'colourlife.core.OthersPropertyFees.createPropertyOrder');
			OthersPropertyFees::model()->addError('id', $result['message']);
			return false;
			//throw new CHttpException(400, '彩之云订单创建失败！');
		}
		//如果收费系统返回的金额为0,那么不创建我们的订单。直接报错
		if ($result['content']['orderamount'] <= 0) {
			Yii::log('彩之云订单创建成功！彩之云订单号:' . $result['content']['orderid'] . '，但返回金额为' . $result['content']['orderamount'] . ',不在创建我们的订单！',
				CLogger::LEVEL_INFO, 'colourlife.core.OthersPropertyFees.createPropertyOrder');
			OthersPropertyFees::model()->addError('id', "彩之云收费系统异常,创建订单失败！" . $result['content']['msg']);
			return false;
		}
		...
		$feeAttr['amount'] = $result['content']['orderamount'];
		//银行支付金额=收费系统返回的金额-我们使用的红包抵扣金额
		$bank_pay = $result['content']['orderamount'] - $feeAttr['red_packet_pay'];
		$feeAttr['bank_pay'] = $bank_pay < 0 ? 0 : $bank_pay;//银行支付不能小于0
		$other->attributes = $feeAttr;

		Yii::log("创建彩之云订单成功，使用彩之云返回金额替换我们订单的金额。物业费订单:{$other->sn},彩之云订单:{$result['content']['orderid']},我们的订单金额	:{$oldAmount},替换后的金额:{$feeAttr['amount']}", CLogger::LEVEL_INFO, 'colourlife.core.propertyfees.create');
	...
	public function callColorcloudOrder($order_id, $state, $note)
	{
		...
		if (1) {//(empty($testCommunityId) || $testCommunityId != $propertyFee->communityId) {
			...
			// ice uuid
			$comm = Community::model()->findByPk(intval($propertyFee->PropertyFees->community_id));
			$uuid = empty($comm) ? null : empty($comm->colorcloudCommunity) ? null : $comm->colorcloudCommunity[0]->color_community_id;
			...
			if (!isset($result['code']) || $result['code'] != 0) { //调用接口修改状态成功
	 ...
	 * @return string
	 */
	public function getBranchNameArr(){
		if (isset($this->customer))
			if (isset($this->customer->community))
				if (isset($this->customer->community->branch)) {
					//return $this->customer->community->branch->name;
					return $this->getMyBranch($this->customer->community->branch->id);
				}
	}
