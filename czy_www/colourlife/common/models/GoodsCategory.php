...
class GoodsCategory extends CActiveRecord
{
    ...
    public function relations()
    {
        ...
        return array(
            'parent' => array(self::BELONGS_TO, 'GoodsCategory', 'parent_id'),
            'children' => array(self::HAS_MANY, 'GoodsCategory', 'parent_id'),
            'enabledChildren' => array(self::HAS_MANY, 'GoodsCategory', 'parent_id', 'condition'=>'`enabledChildren`.state=0'),
        );
    }
    ...
    {
        /* $this->categoryFile = CUploadedFile::getInstance($this, 'categoryFile');*/
    ...
    {
        return $this->findAllByAttributes(array('state' => 0, 'parent_id' => $this->id, 'is_deleted' => 0), array('order' => 'display_order, create_time DESC'));
    }
    ...
    {
        if (Goods::model()->findAll('category_id=:category_id', array(':category_id' => $this->id))) {
    ...
    {
        if (!empty($this->parent_id)&&Goods::model()->findAll('category_id=:category_id', array(':category_id' => $this->parent_id))) {
            $this->addError($attribute, '因为该' . $this->modelName . '下还存在商品' . '，无法进行该操作。');
        }
    }
    ...
    public static function getMyParentCategoryName($myCategoryId = 0, $isOneSelf = false)
    {
        if (empty($myCategoryId)) {
            ...
            $category = GoodsCategory::model()->Enabled()->findByPk($myCategoryId);
            //如果部门不存在或上级部门不存在
            if (empty($category)) {
                return "-";
            } else {
    ...
    {
        $data = array();
        if($model = self::model()->findAll('state = '.Item::STATE_ON)){
