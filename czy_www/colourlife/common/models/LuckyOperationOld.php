...
class LuckyOperation {
    public  $entityList = array(89,90,91,92,93,94,95,96,97,98,106,107,108,109,110,111,112,113,114,115);
	/**
	 * 用户抽奖完成后的更新操作
	 *
	 * @param 用户名 $custname        	
	...
	public function custDoOperation($custname, $custid, $activityid, $prizeid,$redInfo=false, $besideGennerZero = true,$besideid = array(0)) {
		...
		try {
			...
			//var_dump($activityid.'>>>>'.$custid);die;
			if (empty ( $exeRet )) {
				// var_dump(1188);die;
			...
			Yii::log("其他异常错误",CLogger::LEVEL_ERROR,'colourlife.core.LuckyOperation.custDoOperation');
			// catch(Exception $e) {
   //              Yii::log("try异常：".json_encode($e->getMessage()),CLogger::LEVEL_ERROR,'colourlife.core.OrderSendRedPcketCommand');
			// var_dump(4444);die;
			$transaction->rollback ();
			return false;
		}
...
//			//抽奖机会，不关联活动，只关联人，统一放到id为2的活动去。
	...
	public function doLucky($luckyActId,$luckyCustCan,$luckyTodayCan,$userName,$userId,$besideGennerZero = true, $besideid = array(0)){
		...
		//中过泰康人寿的不中，返回谢谢参与
		if($getObj['id']==116){
			$luckModel = LuckyCustResult::model()->find(' cust_id=:cust_id and prize_id in(70,77,86) ',array(':cust_id'=>$userId));
			if($luckModel){
				// $getObj['id']=Item::LUCKY_THANKS_ID;
				$retObj = array (
					'id' => intval(Item::LUCKY_THANKS_ID),
					'prize_name' =>"",
					'all' => 0,
		...
		// 		$getObj['id']=Item::LUCKY_THANKS_ID;
		// 	}else{
		// 		$CustModel = Customer::model()->findByPk($userId);
		// 		if(!$CustModel||$CustModel->community_id!=555){
		// 			$getObj['id']=Item::LUCKY_THANKS_ID;
		// 		}
		// 	}
		// }
